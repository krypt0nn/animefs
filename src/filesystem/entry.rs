...
impl FilesystemTree {
    ...
    pub fn read_root<const BUF_SIZE: u64>(&self) -> FilesystemTreeReader<BUF_SIZE> {
        self.read(Self::ROOT_OFFSET, FilesystemTreeReaderMode::Sibling)
    }
    ...
    pub fn read<const BUF_SIZE: u64>(&self, offset: u64, mode: FilesystemTreeReaderMode) -> FilesystemTreeReader<BUF_SIZE> {
    ...
    pub fn insert_child<const BUF_SIZE: u64>(&mut self, offset: u64, entry: FilesystemEntry) -> u64 {
        ...
        // and update its sibling address.
            ...
            let reader = self.read::<BUF_SIZE>(parent.child_addr, FilesystemTreeReaderMode::Sibling);

            match reader.last() {
                Some((offset, mut child)) => {
                    child.sibling_addr = i;

                    self.book.write(offset, child.to_bytes());
                }
    ...
    pub fn insert_sibling<const BUF_SIZE: u64>(&mut self, offset: u64, entry: FilesystemEntry) -> u64 {
        let reader = self.read::<BUF_SIZE>(offset, FilesystemTreeReaderMode::Sibling);
...
pub mod tests {
    ...
    fn children() {
        with_fs("entry-children", |fs, _| {
            ...
            let (offset, root) = tree.read_root::<24>().next().unwrap();
            let (offset, last_child) = tree.read::<1024>(root.child_addr, FilesystemTreeReaderMode::Sibling).last().unwrap();
